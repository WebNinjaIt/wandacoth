...
function add_landing_page()
{
    ...
    $not_allowed_urls = array( '', '/', 'index.html', 'index.php' );

    if (in_array(gr_post('url'), $not_allowed_urls, true)) {
        wp_send_json_error(['error' => 'Incorrect URL.']);
    }

    if (false === gr_verify_nonce()) {
        wp_send_json_error(['error' => 'Incorrect request.']);
    }
    ...
    wp_send_json_success(['message' => 'Landing page published.']);
}
